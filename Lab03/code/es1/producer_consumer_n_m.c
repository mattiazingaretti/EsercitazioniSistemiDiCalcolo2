...
static inline int performRandomTransaction() {
    ...
    int amount = 1;//rand() % (2 * MAX_TRANSACTION); // {0, ..., 2*MAX_TRANSACTION - 1}
...
int main(int argc, char* argv[]) {
	...
	read_s = (sem_t*) malloc(sizeof(sem_t));
	write_s = (sem_t*) malloc(sizeof(sem_t));
	n = (sem_t*) malloc(sizeof(sem_t));
	e = (sem_t*) malloc(sizeof(sem_t));
	
	int r = sem_init(write_s , 0, 1);
	if (r) handle_error_en(r, "Error in sem_init ! \n ");
	
	r = sem_init(read_s , 0, 1);
	if (r) handle_error_en(r, "Error in sem_init ! \n ");
